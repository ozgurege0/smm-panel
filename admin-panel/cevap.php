<?php 
$cevapid = $_GET['id'];
$cevap = $vt->cek('ASSOC', 'destek', '*', 'where id=?', array($cevapid));
				...
				<div class="col-lg-12">
					<div class="card">
						...
						<div class="card-body">
							...
							<?php 
								$cevaplar = $vt->cek("OBJ_ALL", "destek_cevap", "*", "where destek_id=? and destek_uye_id=? ORDER BY tarih ASC", array($destek['id'],$_SESSION["id"]));
								if($cevaplar){
							...
							if($_POST){
								...
								if(!$mesaj){
									...
									$ekle = $vt->ekle('destek_cevap', 'destek_id,destek_uye_id,cevaplayan,cevap', array($destek['id'], $user['id'], 'admin', $mesaj));
									if($ekle){
										echo '<div class="col-lg-12"><div class="form-group">
										<div class="alert alert-success">Destek talebine cevap yazdınız.</div>
									</div></div>';
										header('refresh:1;url=');
										$guncel = $vt->guncelle('0', 'destek', 'onay', 'where id=?', array(0,$destek['id']));
									}else{
										echo '<div class="col-lg-12"><div class="form-group">
