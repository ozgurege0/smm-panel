		...
		<div class="row">
			<div class="col-sm-12">
				<div class="page-title-box">
					<h4 class="page-title mb-2"><i class="dripicons-folder-open"></i> Kategoriler</h4>  
					<div class="">
						<ol class="breadcrumb">
				...
				<div class="col-12">
					<div class="card">
						<div class="card-body new-user order-list">
							<h4 class="header-title mt-0 mb-3">Kategoriler</h4>
							<?php
							$message = "GOREVLE";
							$kategorisil = @$_POST['kategorisil'];
							if($kategorisil){
					
							  $kategorisilmek = $vt->sorgu("TRUNCATE TABLE kategoriler", array());
							  if($kategorisilmek){
								$message = 'basarili';
							  }else{
								  $message = 'basarili';
							...
							<form method="post" enctype="multipart/form-data" class="form-horizontal">
								<div class="form-group">
									<div class="input-group">
										<input type="submit" class="btn btn-danger btn-block btn-square  waves-effect waves-light" onclick="return confirm('Kategorilerin tümünü silmek istediğinize eminmisiniz?');" name="kategorisil" value="Tümünü Sil">
									</div>
								</div>
							</form>
							<div class="form-group">
								<input type="text" class="form-control" id="ravertan-input" placeholder="Aradığınız kategoriyi yazın..">
							</div>
							<div class="table-responsive">
								<table class="table table-hover mb-0 ravertan-arama">
									...
									<tbody>
                                        <?php 
                                        $uyeler = $vt->cek('OBJ_ALL', 'kategoriler', '*', '', array());
                                        ...
                                        <tr>
                                            ...
                                            <td><?=$row->baslik?></td>
											<td><?=$durum?></td>
											<td><a class="btn btn-info btn-xs" href="kategori-duzenle/<?=$row->id?>">Düzenle</a></td>
											<td><a class="btn btn-danger btn-xs" onclick="return confirm('<?=$row->baslik?> adlı kategoriyi silmek istediğinize eminmisiniz?');" href="kategori-sil/<?=$row->id?>">Sil</a></td>
                                        </tr>
										<?php } ?>
									</tbody>
								</table> <!--end table-->                                               
